...
<script lang="ts">
    ...
    function drawLines() {
        ...
        Object.values(eventsIndex).forEach((subtask: any) => {
            ...
            // Calculate L-shaped line: horizontal extending left from subtask, then vertical to major task
            const subtaskLeftX = subRect.left - svgRect.left; // Left edge of subtask
            const subtaskCenterY = Math.max(subRect.top - svgRect.top + (subRect.height / 2), headerHeight);
            const majorTaskCenterX = majorRect.left - svgRect.left + (majorRect.width / 2);
            const majorTaskBottomY = majorRect.bottom - svgRect.top;
            
            // Horizontal line extends 20px to the left of subtask, then aligns with major task center
            const horizontalStartX = subtaskLeftX;
            const horizontalMidX = subtaskLeftX - 20; // Extend left
            const verticalX = majorTaskCenterX; // Align with major task center

            // Don't draw if subtask is scrolled above header
            if (subRect.top < svgRect.top + headerHeight) return;

            console.log('Drawing L-shaped line from subtask', subtask.id);
            ...
            // Create first horizontal segment (subtask to the left)
            const horizontalLine1 = document.createElementNS('http://www.w3.org/2000/svg', 'line');
            horizontalLine1.setAttribute('x1', horizontalStartX.toString());
            horizontalLine1.setAttribute('y1', subtaskCenterY.toString());
            horizontalLine1.setAttribute('x2', horizontalMidX.toString());
            horizontalLine1.setAttribute('y2', subtaskCenterY.toString());
            horizontalLine1.setAttribute('stroke', color);
            horizontalLine1.setAttribute('stroke-width', '3');
            horizontalLine1.setAttribute('opacity', '0.8');
            horizontalLine1.setAttribute('pointer-events', 'none');
            svg.appendChild(horizontalLine1);

            // Create vertical line (from left extension up to major task level)
            const verticalLine = document.createElementNS('http://www.w3.org/2000/svg', 'line');
            verticalLine.setAttribute('x1', horizontalMidX.toString());
            verticalLine.setAttribute('y1', subtaskCenterY.toString());
            verticalLine.setAttribute('x2', horizontalMidX.toString());
            verticalLine.setAttribute('y2', majorTaskBottomY.toString());
            ...
            // Create second horizontal segment (from vertical line to major task center)
            const horizontalLine2 = document.createElementNS('http://www.w3.org/2000/svg', 'line');
            horizontalLine2.setAttribute('x1', horizontalMidX.toString());
            horizontalLine2.setAttribute('y1', majorTaskBottomY.toString());
            horizontalLine2.setAttribute('x2', verticalX.toString());
            horizontalLine2.setAttribute('y2', majorTaskBottomY.toString());
            horizontalLine2.setAttribute('stroke', color);
            horizontalLine2.setAttribute('stroke-width', '3');
            horizontalLine2.setAttribute('opacity', '0.8');
            horizontalLine2.setAttribute('pointer-events', 'none');
            ...
            <div class="time-row" style="height: {pixelsPerHour}px;">
                ...
                {#each weekDates as date, dayIndex}
                    ...
                    <div
                            ...
                            on:keydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleCellClick(dateStr, timeSlot); } }}
                            role="gridcell"
                            tabindex="0"
                    >
                        <!-- Empty timeline cell -->
